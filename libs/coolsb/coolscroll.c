...
//
static int DrawScrollArrow(SCROLLBAR *sbar, HDC hdc, RECT *rect, UINT arrow, BOOL fMouseDown, BOOL fMouseOver, SCROLLWND* sw)
{
	UINT ret;
	UINT flags = arrow;
	...
	if(sbar->fFlatScrollbar != CSBS_NORMAL)
	{
		...
		if(fMouseDown)
		{
			SetBkColor(hdcmem2, 0);
			BitBlt(hdcmem1, 0, 0, width, height, hdcmem2, 0, 0, SRCCOPY);
			SetBkColor(hdc, sw->hottracked);
			SetTextColor(hdc, sw->background);// GetSysColor(COLOR_3DDKSHADOW));
		...
		else if(fMouseOver)
		{
			SetBkColor(hdcmem2, 0);
			BitBlt(hdcmem1, 0, 0, width, height, hdcmem2, 0, 0, SRCCOPY);
			SetBkColor(hdc, sw->foreground);
			SetTextColor(hdc, sw->background);
			BitBlt(hdc, rect->left, rect->top, width, height, hdcmem1, 0, 0, SRCCOPY);

			//SetBkColor(hdcmem2, sw->background);// GetSysColor(COLOR_BTNTEXT));
...
static void DrawThumb(HDC hdc, RECT *rect, COLORREF backgnd, COLORREF foreground, BOOL horz)
{
	PaintRect(hdc, rect, backgnd);
	RECT r = *rect;
...
static LRESULT NCDrawHScrollbar(SCROLLBAR *sb, HWND hwnd, HDC hdc, const RECT *rect, UINT uDrawFlags)
{
	...
	if (scrollwidth > butwidth*2)
	{
		...
		if(fCustomDraw)
			...
			DrawScrollArrow(sb, hdc, &ctrl, uLeftButFlags, fMouseDownL, fMouseOverL, sw);
		...
		{	
			...
			{
				PaintRect(hdc, &sbm, sw->background);
			...
			else
			{
				...
				{	
					//PaintRect(hdc, &thumb, sw->hottracked);// GetSysColor(COLOR_3DSHADOW));
					DrawThumb(hdc, &thumb, sw->background, sw->hottracked, sb->nBarType == SB_HORZ);
				}
				else
#endif
				{
					//DrawBlankButton(hdc, &thumb, uDEFlat);
					//PaintRect(hdc, &thumb, sw->foreground);
					DrawThumb(hdc, &thumb, sw->background, sw->foreground, sb->nBarType == SB_HORZ);
				}
			}
			RotateRect0(sb, &thumb);
		...
		if(fCustomDraw)
			...
			DrawScrollArrow(sb, hdc, &ctrl, uRightButFlags, fMouseDownR, fMouseOverR, sw);
...
static LRESULT ThumbTrackHorz(SCROLLBAR *sbar, HWND hwnd, int x, int y)
{
	...
	if (fCustomDraw)
		...
		PaintRect(hdc, &rc2, sw->background);
	...
	else
	{
		...
		if(sbar->fFlatScrollbar)
			//PaintRect(hdc, &rc2, sw->foreground);// GetSysColor(COLOR_3DSHADOW));
			DrawThumb(hdc, &rc2, sw->background, sw->hottracked, sbar->nBarType == SB_HORZ);
		else
#endif
		{
	...
	if(siMaxMin > 0)
		pos = si->nMin + MulDiv(thumbpos - rc.left, siMaxMin - si->nPage + 1, rc.right - rc.left - nThumbSize);
	else
...
static LRESULT CoolSB_Timer(SCROLLWND *swnd, HWND hwnd, WPARAM wTimerId, LPARAM lParam)
{
	...
	else if(wTimerId == COOLSB_TIMERID2)
	{
		...
		if(uScrollTimerPortion != HTSCROLL_NONE)
			SendScrollMessage(hwnd, uScrollTimerMsg, uScrollTimerPortion | HTSCROLL_REPEATED, 0);
...
BOOL WINAPI CoolSB_SetColors(HWND hwnd, COLORREF background, COLORREF foreground, COLORREF hottracked)
{
	SCROLLWND* sw = 0;
	if ((sw = GetScrollWndFromHwnd(hwnd)) == NULL)
		return FALSE;
	...
	sw->foreground = foreground;
	sw->hottracked = hottracked;
